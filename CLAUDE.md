...
- Native bridge only on Linux: `./Ctp.Bridge.C/build.sh`
- Native bridge only on Windows: `./Ctp.Bridge.C/build.ps1 -CtpSdkRoot <path-to-ctp-sdk>`
...
- `Ctp.Bridge.Net` triggers `Ctp.Bridge.C/build.sh` or `build.ps1` from its MSBuild targets, so building managed projects also rebuilds and copies the native bridge artifacts.
...
This repo is a three-layer wrapper around the official CTP SDK:

1. `Ctp.Bridge.C` is the native C++ layer. It links the vendor SDK and exposes a stable C ABI in `include/ctp_bridge.h`, implemented by `src/md_bridge.cpp` and `src/trader_bridge.cpp`.
2. `Ctp.Bridge.Net` is the low-level F# interop layer. It owns `DllImport` declarations, native struct layouts, callback delegate registration, `SafeHandle` lifetimes, and marshalling between fixed-size native buffers and managed records.
3. `Ctp.Net` is the public F# API. It turns callback-driven native APIs into `Async<Result<...>>` methods and .NET events for market data and trader workflows.

## Bundled CTP SDK layout

- `Ctp.Bridge.C/ctp-sdk/<version_date>/mduserapi` contains the market-data SDK headers and native libraries.
- `Ctp.Bridge.C/ctp-sdk/<version_date>/traderapi` contains the trader SDK headers and native libraries.
- `Ctp.Bridge.C/ctp-sdk/<version_date>/reference` contains the official CTP API reference documentation bundled with that SDK version.
...
- `Ctp.Bridge.Net/Common.fs` is the shared interop spine: native library resolution, encoding defaults, fixed-width string marshalling, and shared request/response/native struct helpers.
- `Ctp.Bridge.Net/MdBridge.fs` and `Ctp.Bridge.Net/TraderBridge.fs` mirror the C ABI for the market-data and trader APIs separately. They map native callbacks into F# records and expose thin `MdApi` / `TraderApi` wrappers.
...
- The market-data and trader stacks are intentionally parallel across all layers. Adding or changing a CTP field or API usually requires coordinated edits in:
  1. the official CTP API reference under `Ctp.Bridge.C/ctp-sdk/<version_date>/reference` should be consulted first to confirm field sizes, callback contracts, request/response semantics, and API-specific constraints
  2. `Ctp.Bridge.C/include/ctp_bridge.h`
  3. the corresponding C++ bridge implementation
  4. the matching F# native structs/imports/mappers in `Ctp.Bridge.Net`
